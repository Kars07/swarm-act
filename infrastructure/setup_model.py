...
# Define the production application
app = modal.App("bct-swarm-architecture")
# 1. Image Definition: Stripped down, no heavy training frameworks.
# We include bitsandbytes for NF4 and flash-attn for the Gated Attention blocks.
# 1. Image Definition: Upgraded to a CUDA-devel base image
swarm_image = (
    modal.Image.from_registry("pytorch/pytorch:2.3.0-cuda12.1-cudnn8-devel")
    .apt_install("git", "wget", "build-essential")
    .pip_install("packaging", "ninja") # Core build dependencies
    .pip_install(
        "flash-attn",
        "transformers>=4.40.0",
        "accelerate",
...
# 2. Persistent Storage: Zero memory duplication across cold starts
# This volume will mount to /workspace/data in our containers
storage_volume = modal.Volume.from_name("bct-swarm-storage", create_if_missing=True)
CACHE_DIR = "/workspace/data/model_cache"
DATA_DIR = "/workspace/data/raw_interactions"

# 3. Model Ingestion Function
@app.function(
    image=swarm_image,
    volumes={"/workspace/data": storage_volume},
    timeout=3600, # 1 hour timeout for initial download
    secrets=[modal.Secret.from_name("huggingface-secret")] # Ensure your HF token is in Modal secrets
)
def ingest_qwen_model(model_id: str = "Qwen/Qwen3.5-4B"):
    from huggingface_hub import snapshot_download

    print(f"Initializing download for {model_id}...")
    print("Targeting 4B Parameters | 32 Layers | Gated DeltaNet Hybrid")

    # Download directly to the persistent volume
    snapshot_download(
        repo_id=model_id,
        local_dir=f"{CACHE_DIR}/{model_id.replace('/', '_')}",
        local_dir_use_symlinks=False,
        ignore_patterns=["*.msgpack", "*.h5", "coreml/*"] # Exclude unnecessary formats
    )

    print("Download complete. Model secured in Modal Volume.")

    # Verify the architecture config matches the 248320 token embedding expectation
    import json
    config_path = f"{CACHE_DIR}/{model_id.replace('/', '_')}/config.json"
    if os.path.exists(config_path):
        with open(config_path, 'r') as f:
            config = json.load(f)
            print(f"Verified Vocab Size: {config.get('vocab_size', 'Unknown')}")
...
def build_swarm_dataset():
    ...
    for i, chunk in enumerate(chunks):
        ...
        # The Fix: explicitly include 'word_count' so it survives the concatenation
        df_list.append(chunk[['user_id', 'business_id', 'stars', 'text', 'useful', 'date', 'word_count']])
